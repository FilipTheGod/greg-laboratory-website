...
import { createCheckout, addItemToCheckout, updateCheckoutItem } from '@/lib/shopify';
    ...
    const initializeCheckout = async () => {
      ...
      if (existingCheckoutId) {
        ...
        try {
          ...
          // Restore cart items if they exist
          if (existingCartItems) {
            const items = JSON.parse(existingCartItems);
            setCartItems(items);
            ...
            if (items.length > 0) {
              ...
              const updatedCheckout = await addItemToCheckout(checkout.id, lineItems);
