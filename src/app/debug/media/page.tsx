...
export default async function MediaDebugPage() {
  ...
  try {
    ...
    const productsWithVideo = products.filter(p =>
      p.media &&
      p.media.length > 0 &&
      ...
      <div className="p-8">
        ...
        <div className="mb-6">
          ...
          <ul className="list-disc pl-5">
            ...
            <li>Total video media items: {productVideos.reduce((acc, p) => acc + p.videos.length, 0)}</li>
          </ul>
        </div>
    ...
    return (
      <div className="p-8">
        <h1 className="text-2xl mb-4">Media Debug Page</h1>
        <p className="text-red-600">Error loading products: {error instanceof Error ? error.message : String(error)}</p>
      </div>
    );
  }
}